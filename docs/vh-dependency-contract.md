...
Special rule:
- If `context` contains the parent key with empty value (`''`/`null`), CAP treats it as explicit empty and does not fallback to persisted data.
...
- requested `fieldCode`
- each dependency and resolved source (`context`, `persisted`, `none`)
